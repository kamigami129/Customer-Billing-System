...
typedef struct { 
    ...
    char date[20]; float total_amount; 
    int is_registered; 
} Transaction;
...
void edit_product() {
    ...
    printf("Current: %s | $%.2f | Stock: %d\n", products[idx].name, products[idx].price, products[idx].stock);
    float new_price; printf("New price (0 to skip): $"); scanf("%f", &new_price);
